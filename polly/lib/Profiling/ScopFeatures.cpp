...
static isl_stat evalPieceCB(isl_set *Set, isl_qpolynomial *Qp, void *User) {
  ...
  // TODO: if this becomes a problem, we could generate IR to check the
  // condition and select 0 vs. eval(Qp) at runtime.
  isl_set_free(Set);
...
static isl_stat evalUnionPieceCB(isl_pw_qpolynomial *PwQp, void *User) {
  ...
  // If any piece fails, propagate -1
  if (V == MinusOne) {
    Ctx->Failed = true;
    return isl_stat_error;
  }
...
// ---------------------------------------------------------------------------
// Public: computeTripCountIR
// ---------------------------------------------------------------------------

Value *polly::computeTripCountIR(const Scop &S, IRBuilder<> &B,
                                 Instruction * /*InsertBefore*/) {
  Type *I64 = B.getInt64Ty();
  Value *MinusOne = ConstantInt::get(I64, (uint64_t)-1LL);

#ifdef POLLY_HAVE_BARVINOK
  // Build id → Value* map for SCoP parameters.
  DenseMap<isl_id *, Value *> IdToValue;
  ...
  // Compute the Ehrhart polynomial for the full iteration domain.
